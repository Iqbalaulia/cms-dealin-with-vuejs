<template>
  <div class="verif_idcard">
    <Table
      ref="table"
      :tableData="verificationIdCardHistory.items"
      :columns="verificationIdCardHistory.columns_data"
      :listeners="listeners"
    >
...
export default {
  ...
  computed: {
    ...mapState({
      verificationIdCardHistory: (state) =>
        state.verificationIdCardHistory.data,
    }),
    listeners() {
      return {
        ...this.$listeners,
  ...
  methods: {
    ...mapActions("verificationIdCardHistory", [""]),
    openModal() {
      this.showdataform = true;
    },
    openDetails(row) {
      this.$router.push(`/verification/id-card/${row.id}/history`);
      console.log(`---rw`, row);
    },
  },
  components: {
